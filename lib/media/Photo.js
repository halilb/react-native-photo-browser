...
import mimetype from 'react-native-mimetype';
...
export default class Photo extends Component {
  ...
  _renderSelectionButton() {
   ...
   downloadAndOpen(url, contentType) {
    var path = RNFS.ExternalDirectoryPath + url.substring(url.lastIndexOf("/") + 1);
    RNFS.exists(path).then(function(doesExists) {
        if(doesExists) {
          FileOpener.open(
                              path,
                              contentType
                              ...
                              ToastAndroid.show("Could not open file : " + contentType, ToastAndroid.SHORT)
                          })
        }else {
          ...
          xhr.onload = function(e) {
            if (this.status == 200) {
              // write the file
              RNFS.writeFile(path, this._response, 'base64')
                .then((success) => {
                  FileOpener.open(
                              path,
                              contentType
                ...
                .catch((err) => {
                  console.log("Could not write the file : " + path);
  ...
  render() {
    ...
    if(source && source.uri) {
      var contentType = mimetype.lookup(source.uri);
      switch(contentType) {
        case 'application/pdf' :
                  return (
                            <TouchableHighlight onPress={this.downloadAndOpen.bind(this, source.uri, contentType)}>
                                <View style={[styles.container, sizeStyle]}>
                                  <FontAwesomeIcon name="file-pdf-o" size={30} color="#900" />
