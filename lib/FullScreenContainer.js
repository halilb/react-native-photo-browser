...
export default class FullScreenContainer extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.state = {
      dataSource: dataSource.cloneWithPages(props.mediaList),
  ...
  _updatePageIndex(index) {
    this.setState({
      ...
      const newTitle = `${index + 1} of ${this.props.mediaList.length}`;
  ...
  _onPreviousButtonTapped() {
    ...
    if (prevIndex < 0) {
      prevIndex = this.state.mediaList.length - 1;
    }
  ...
  _renderRow(media: Object, rowID: number) {
    const {
  ...
  _renderScrollableContent() {
    ...
    return (
      <ViewPager
        ...
        renderPage={this._renderRow}
        onChangePage={this._onPageSelected}
      />
    );
  }
