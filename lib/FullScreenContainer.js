...
export default class FullScreenContainer extends React.Component {
  ...
  constructor(props, context) {
    ...
    this._onPhotoLongPressed = this._onPhotoLongPressed.bind(this);
  ...
  _onPhotoLongPressed() {
    const onPhotoLongPressed = this.props.onPhotoLongPress;

    // action behaviour must be implemented by the client
    // so, call the client method or simply ignore this event
    if (undefined !== this.props.onPhotoLongPress) {
      const { currentMedia, currentIndex } = this.state;
      onPhotoLongPressed(currentMedia, currentIndex);
    }
  }
