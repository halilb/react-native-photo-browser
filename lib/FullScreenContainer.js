...
export default class FullScreenContainer extends React.Component {
  ...
  _updatePageIndex(index) {
    this.setState({
      ...
      this.props.updateTitle(newTitle, index);
    });
  }
      ...
      <View key={`row_${rowID}`} style={styles.flex}>
        <TouchableWithoutFeedback
          ...
          <Photo
            ...
            onLoad={() => {
              if (this.props.dataSource && this.props.prefetchNext && rowID < this.props.dataSource.getRowCount() - 1) this._triggerPhotoLoad(Number.parseInt(rowID, 10) + 1)
            }}
