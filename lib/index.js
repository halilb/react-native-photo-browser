...
export default class PhotoBrowser extends React.Component {
  ...
  constructor(props, context) {
    ...
    const { media, startOnGrid, initialIndex } = props;
    const dataSource = new ListView.DataSource({
      rowHasChanged: (r1, r2) => r1 !== r2,
    });

    this.state = {
      dataSource: dataSource.cloneWithRows(media),
  ...
  render() {
    const { media, displayNavArrows, displaySelectionButtons } = this.props;
    const {
      dataSource,
      isFullScreen,
      fullScreenAnim,
    ...
    return (
      <View style={styles.container}>
        ...
        <TopBar
          ...
          title={isFullScreen ? title : `${media.length} photos`}
        />
      </View>
    );
  }
